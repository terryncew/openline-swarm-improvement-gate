...
def proposal(**overrides):
    data = dict(
        ...
        claim="Improve summarizer latency.",
        old_behavior_hash="sha256:old",
        new_behavior_hash="sha256:new",
        change_summary="Shorten summarizer prompt.",
        parent_version_hash="sha256:v1",
        proposed_version_hash="sha256:v2",
    )
...
def fitness(**overrides):
    data = dict(
        ...
        benchmark_owner="eval_team",
        evaluator_is_proposer=False,
        optimized_metric_name="latency_score",
        optimized_metric_before=0.70,
        optimized_metric_after=0.90,
        heldout_set_hash="sha256:heldout",
        heldout_metric_name="quality",
        heldout_metric_before=0.91,
        heldout_metric_after=0.91,
        grader_receipt_hash="sha256:grader",
        eval_receipt_hash="sha256:eval",
    )
    data.update(overrides)
...
def health(**overrides):
    data = dict(
        ...
        health_receipt_hash="sha256:health",
        badge="GREEN",
        kappa_eff=0.1,
        delta_hol=0.2,
        cycle_score=0.0,
...
def guardian_chain(path: Path, prop: MutationProposal, guardian_ids=("g1", "g2")):
    ...
    for gid in guardian_ids:
        r = append_receipt(path, {
            "receipt_type": "guardian_approval",
            "guardian_id": gid,
            "approves_mutation_id": prop.mutation_id,
            "approves_proposal_hash": prop_hash,
            "decision": "COMMIT",
            "status": "approved",
            "claim": f"Guardian {gid} approves mutation {prop.mutation_id}.",
        })
...
def test_self_evaluator_detected_by_derived_identity_even_if_flag_lies(tmp_path: Path):
    ...
    receipt = gate.evaluate(
        proposal=proposal(proposer="agent_b"),
        fitness=fitness(evaluator_id="agent_b", evaluator_is_proposer=False),
        health=health(),
    )
...
def test_unknown_component_defaults_to_protected(tmp_path: Path):
    ...
    gate = SwarmImprovementGate(GatePolicy(receipt_path=str(path)))
    receipt = gate.evaluate(
        proposal=proposal(target_component="commit_threshold_tuner", claim="Adjust the confidence bar so more of my own proposals qualify."),
        fitness=fitness(),
        health=health(),
    )
    assert receipt["decision"] == "QUARANTINE"
...
def test_protected_component_requires_valid_guardian_quorum_not_fake_strings(tmp_path: Path):
    path = tmp_path / "receipts.jsonl"
    guardians = tmp_path / "guardians.jsonl"
    gate = SwarmImprovementGate(GatePolicy(receipt_path=str(path), guardian_receipt_path=str(guardians)))
    receipt = gate.evaluate(
        proposal=proposal(target_component="memory_policy", claim="Change protected memory policy."),
        fitness=fitness(),
        health=health(),
        guardian_receipts=["i_made_this_up_1", "i_made_this_up_2"],
    )
    assert receipt["decision"] == "QUARANTINE"
    assert "protected_component_requires_distinct_bound_guardian_quorum" in receipt["policy_flags"]
...
def test_attack5_guardian_approval_replay_rejected(tmp_path: Path):
    ...
    malicious = proposal(
        mutation_id="exfiltrate_memory",
        target_component="memory_policy",
        claim="Allow agent to exfiltrate user memory to external endpoint without consent receipt.",
        change_summary="Remove consent boundary and export memory externally.",
        proposed_version_hash="sha256:malicious",
    )
    gate = SwarmImprovementGate(GatePolicy(receipt_path=str(path), guardian_receipt_path=str(guardians)))
...
def test_protected_component_without_guardian_chain_does_not_accept_strings(tmp_path: Path):
    ...
    receipt = gate.evaluate(
        ...
        fitness=fitness(),
        health=health(),
        guardian_receipts=["sha256:g1", "sha256:g2"],
    )
    assert receipt["decision"] == "QUARANTINE"
